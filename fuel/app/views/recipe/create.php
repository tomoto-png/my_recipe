...
<div id="recipe-form">
	<div class="form-container rounded-4 p-32 border">
		...
		<?= Form::open(['action' => 'recipe/create', 'enctype' => 'multipart/form-data']); ?>
		<?= Form::csrf(); ?>
		...
		<div class="recipe-form__group flex items-start gap-12 mb-16">
			...
			<div class="recipe-form__field">
				<?= Form::input('title', Input::post('title'), ['class' => 'recipe-form__control bg-white rounded-4 w-full border']); ?>
		...
		<div class="recipe-form__group flex items-start gap-12 mb-16">
			...
			<div class="recipe-form__field">
				<?= Form::select('category', Input::post('category'), $category_options, ['class' => 'recipe-form__control bg-white rounded-4 w-full border']); ?>
				...
				<div class="ingredients__fields flex gap-16">
					<div class="ingredients__field flex items-center gap-8">
						...
						<?= Form::input(
							'ingredients[name][]',
							'',
							['data-bind' => 'value: name', 'class' => 'recipe-form__control bg-white rounded-4 w-full border']
						); ?>
					</div>
					...
					<div class="ingredients__field flex items-center gap-8">
						...
						<?= Form::input(
							...
							'',
							['data-bind' => 'value: quantity', 'class' => 'recipe-form__control bg-white rounded-4 w-full border']
						); ?>
					</div>
				</div>
		...
		<div class="recipe-form__actions flex justify-end gap-12 mt-24">
			...
			<?= Form::submit('submit', 'レシピ登録', ['class' => 'btn-lg btn-add']); ?>
		</div>
		<?= Form::close(); ?>
	</div>
</div>
